...
Validates:
  - every *.yaml under examples/ against schemas/factory.schema.json, except
    files named campaign.yaml (schemas/campaign.schema.json), files under a
    guarantees/ directory (schemas/guarantee.schema.json), and the worked
    fixtures under examples/fixtures/ (checked by their own passes below)
  - examples/fixtures/*/campaign.yaml against schemas/campaign.schema.json
  - examples/fixtures/**/work-manifests/*.json against
    schemas/work-manifest.schema.json
  - docs/observability/sample-events.jsonl parses as JSONL and every event
    carries the identity fields (work_id, a timestamp key, an event-kind key)
...
def check_examples(results):
    ...
    files = [p for p in files if "fixtures" not in p.relative_to(examples_dir).parts]
    if not files:
...
def check_fixture_campaigns(results):
    if not FIXTURES_DIR.exists():
        results.append((False, "examples/fixtures/", "directory missing"))
        return
    files = sorted(FIXTURES_DIR.glob("*/campaign.yaml"))
    if not files:
        results.append((False, "examples/fixtures/*/campaign.yaml", "no files found"))
        return
    for path in files:
...
def check_work_manifests(results):
    ...
    if not files:
        results.append((False, "examples/fixtures/**/work-manifests/*.json",
                        "no files found"))
        return
    for path in files:
...
def check_sample_events(results):
    path = REPO_ROOT / "docs" / "observability" / "sample-events.jsonl"
    if not path.exists():
        results.append((False, "docs/observability/sample-events.jsonl",
                        "file missing"))
        return
    problems = []
    count = 0
