...
Usage:
    python3 src/checks/prose_check.py [--warn-only]
...
# docs/style.md quotes the banned-phrase list, so it is exempt from the
# phrase rules. AGENTS.md and CLAUDE.md carry tool-managed blocks (written by
# bd init and the agent harness) whose wording contributors do not control, so
# the phrase rules are relaxed for them too. The em dash rule applies to all
# three; docs/style.md declares it banned everywhere in the repository.
PHRASE_EXEMPT_RELPATHS = {"docs/style.md", "AGENTS.md", "CLAUDE.md"}
