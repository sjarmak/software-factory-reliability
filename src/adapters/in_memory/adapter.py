...
dicts shaped like src/adapters/protocol.schema.json commands; each call returns
one observation dict. An optional stdio loop
(python3 src/adapters/in_memory/adapter.py) reads one command per line and
writes one observation per line.
...
# src/, so the absolute import below would raise ModuleNotFoundError. Put src/
# on the path when it is missing, so the documented invocation needs no
# PYTHONPATH and no install step.
if __package__ in (None, ""):  # pragma: no cover - script-invocation path
    sys.path.insert(0, str(Path(__file__).resolve().parents[2]))

from adapters.in_memory.model import (  # noqa: E402
    DRIFTED_STATE, TERMINAL_OUTCOMES, Factory)
